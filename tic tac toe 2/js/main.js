...
const computerMove = (board) => {
  ...
  allPossibleMoves.forEach((elem) => {
    ...
    let score = minimax(board, true, 0, -positionScore, positionScore);
...
function update(winner) {
  ...
  if (winner === "X") {
    $(".text").html("AI won the game!");
  } else if (winner === "O") {
    $(".text").html("Human player won the game!");
  } else if (winner === 0) {
    $(".text").html("It is a draw");
  } else {
    $(".text").html("Your turn");
    console.log("Time taken for the search: " + timeTaken);
  }
}
